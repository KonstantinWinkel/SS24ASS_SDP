...
if os.path.isfile("calibration.yaml"):
    if click.confirm('\nFile calibration.yaml exists. Overwrite?', default=True):
...
for i in range(len(pts_all_left)):
    crt_pts_l = pts_all_left[i]
    crt_pts_r = pts_all_right[i]
    img = cv2.imread(image_order[i])
    distsum = 0
    for j in range(len(crt_pts_l)):
        distance = DLT(P1, P2, crt_pts_l[j], crt_pts_r[j])[2]
        distsum += distance
        cv2.putText(img, str(distance), (int(crt_pts_l[j][0]), int(crt_pts_l[j][1])), font, 0.2, (0, 0, 255), 1, cv2.LINE_AA)
    cv2.imshow(image_order[i]+" avg dist="+str(distsum/len(crt_pts_l)), img)
    cv2.waitKey(0)
